...
def Grafica(xgt,ygt,xgj,ygj,tg,N):     
     ...
     l1.plot(xgt[(i-1)//int(tf)],ygt[(i-1)//int(tf)],"o",color = "green", label = "Sol",ms = 4)
     l1.plot(xgj[i-1],ygj[i-1],"o",color = "red", label = "Sol",ms = 6)
     l1.plot(xgt[:(i-1)//int(tf)],ygt[:(i-1)//int(tf)],"--", color = "green",ms = 1,lw = 1)
...
Metodo_De_Euler(x0t,y0t,vx0t,vy0t,mt,x0j,y0j,vx0j,vy0j,mj,ms,t0,tf,h,f"Metodo de Euler Cromer\nEl Movimiento Planetario 3")